...
function applyRendererClearColor(
...
interface LightPillarProps {
  /** Canvas clear color (e.g. `#ffffff`). Omit or `transparent` for a fully transparent WebGL backdrop. */
...
const LightPillar: React.FC<LightPillarProps> = ({
  ...
  useEffect(() => {
    ...
    applyRendererClearColor(renderer, backgroundColor);
    ...
    const fragmentShader = `
      ...
      uniform float uBlendBackground;
      varying vec2 vUv;
      ...
      void main() {
        ...
        vec3 col = color * uIntensity;
        if (uBlendBackground > 0.5) {
          col = clamp(uBackgroundColor + col, 0.0, 1.0);
        }
    ...
    const hasSolidBg =
      Boolean(backgroundColor) && backgroundColor !== "transparent";
    ...
    const material = new THREE.ShaderMaterial({
      ...
      uniforms: {
        ...
        uBackgroundColor: {
          value: hasSolidBg
            ? parseColor(backgroundColor as string)
            : new THREE.Vector3(0, 0, 0),
        },
        uBlendBackground: { value: hasSolidBg ? 1.0 : 0.0 },
      },
      transparent: true,
      depthWrite: false,
      depthTest: false,
  ...
  useEffect(() => {
    ...
    applyRendererClearColor(rendererRef.current, backgroundColor);
  }, [backgroundColor]);
  ...
  useEffect(() => {
    ...
    const hasSolidBg =
      Boolean(backgroundColor) && backgroundColor !== "transparent";
    materialRef.current.uniforms.uBackgroundColor.value = hasSolidBg
      ? parseColor(backgroundColor as string)
      : new THREE.Vector3(0, 0, 0);
    materialRef.current.uniforms.uBlendBackground.value = hasSolidBg ? 1.0 : 0.0;
  }, [backgroundColor]);

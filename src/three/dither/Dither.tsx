...
void main() {
  ...
  vec3 col = mix(bgColor, waveColor, t);
...
interface WaveUniforms {
  ...
  bgColor: THREE.Uniform<THREE.Color>;
  invertPattern: THREE.Uniform<number>;
...
interface DitheredWavesProps {
  ...
  /** Color where the wave pattern is “low” (replaces black). */
  backgroundColor: [number, number, number];
  /** Swap low/high so ripples read inverted vs dark mode. */
...
function DitheredWaves({
  ...
  const waveUniformsRef = useRef<WaveUniforms>({
    ...
    bgColor: new THREE.Uniform(new THREE.Color(...backgroundColor)),
    invertPattern: new THREE.Uniform(invertPattern ? 1 : 0),
  ...
  useFrame(({ clock }) => {
    ...
    if (!prevBg.current.every((v, i) => v === backgroundColor[i])) {
      u.bgColor.value.set(...backgroundColor);
      prevBg.current = [...backgroundColor];
    }

    u.invertPattern.value = invertPattern ? 1 : 0;
...
interface DitherProps {
  ...
  waveAmplitude?: number;
  waveColor?: [number, number, number];
  /** Base color where the noise is low (default black). Use a light tint in light mode. */
  backgroundColor?: [number, number, number];
  /** If true, uses `1 - pattern` so bright/dark regions swap (optional “inverse” look). */
  invertPattern?: boolean;
  colorNum?: number;
  pixelSize?: number;
  disableAnimation?: boolean;
